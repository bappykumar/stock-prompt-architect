...
const CustomDropdown = ({ 
  ...
  return (
    <div className={`w-full ${disabled ? 'opacity-30 pointer-events-none grayscale' : 'opacity-100'}`} ref={dropdownRef}>
      ...
      <div className="relative">
        ...
        {isOpen && (
          <div className="absolute z-[120] w-full mt-2 bg-white border border-slate-200 rounded-2xl shadow-[0_12px_32px_rgba(0,0,0,0.1)] overflow-hidden animate-in">
...
export default function App() {
  ...
  return (
    <div className="flex h-screen overflow-hidden font-sans">
      ...
      <aside className="w-[340px] border-r border-slate-200/80 bg-white flex flex-col shrink-0 overflow-y-auto custom-scrollbar relative z-40 pt-16">
        <div className="p-8 pb-12 flex flex-col gap-8">
          
          {/* Section: Custom Architect Input */}
          <section className="bg-slate-50/50 p-5 rounded-[20px] border border-slate-200/60 flex flex-col gap-4 shadow-sm">
          ...
          {/* Section: Seasonality */}
          <section className="bg-slate-50/50 p-5 rounded-[20px] border border-slate-200/60 flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2 text-slate-900">
          ...
          {/* CTA BLOCK */}
          <div className="pt-4 flex flex-col gap-3 sticky bottom-0 bg-white pb-8">
            <button 
              onClick={handleGenerate}
              disabled={isGenerating}
              className="w-full py-3.5 bg-slate-900 text-white rounded-xl text-[12px] font-bold uppercase tracking-wider hover:bg-slate-800 disabled:opacity-30 transition-all shadow-sm active:scale-[0.98] flex items-center justify-center gap-2.5"
            >
              {isGenerating ? (
      ...
      {/* Main Workspace */}
      <main className="flex-1 overflow-y-auto custom-scrollbar relative flex flex-col pt-16">
        ...
        <div className={`flex-1 max-w-4xl w-full mx-auto px-8 py-12 transition-all duration-500 ${isGenerating ? 'opacity-40' : 'opacity-100'}`}>
          {batches.length > 0 ? (
                ...
                <section key={batch.id} className="animate-in" style={{ animationDelay: `${batchIdx * 80}ms` }}>
                  ...
                  <div className="grid gap-6">
                    {batch.prompts.map((p, pIdx) => (
                      <div 
                        key={p.id} 
                        className="group relative bg-white rounded-3xl p-8 border border-slate-100 shadow-sm hover:shadow-md hover:border-slate-200 transition-all flex flex-col gap-6"
                      >
                        <div className="flex items-center justify-between">
                        ...
                        <p className="text-[15px] font-medium leading-[1.7] text-slate-700 group-hover:text-slate-900 transition-colors">
                          {p.text}
                        </p>
                        ...
                        <div className="flex items-center justify-end pt-4 border-t border-slate-50">
                          <button 
                            ...
                            className={`px-8 py-3 rounded-xl text-[11px] font-bold uppercase tracking-widest transition-all shadow-sm active:scale-95 flex items-center justify-center gap-2.5 ${p.copied ? 'bg-emerald-500 text-white' : 'bg-slate-900 text-white hover:bg-slate-800'}`}
                          >
                            {p.copied ? (
                              <><Check size={14} strokeWidth={3} /><span>Copied</span></>
                            ) : (
            ...
            <div className="h-full flex flex-col items-center justify-center text-center animate-in min-h-[50vh]">
              ...
              <p className="text-[14px] font-medium text-slate-400 max-w-sm leading-relaxed mb-12">
                Configure your vision in the control panel to generate high-performance stock prompts.
              </p>
